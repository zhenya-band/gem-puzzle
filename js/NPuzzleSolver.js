...
NPuzzleSolver.prototype.solveColumn = function (size) {
  ...
  if (this.numbers[secondToLast].x !== colNumber
    ...
    this.applyRelativeMoveList(pos, moveList);
    // ow the order has been officially reversed
  }
  // do the special
...
NPuzzleSolver.prototype.makeEmptyNeighborTo = function (num) {
  const gotoPos = this.numbers[num];
  let counter = 1;
  while ((this.numbers[0].x !== gotoPos.x || this.numbers[0].y !== gotoPos.y)
